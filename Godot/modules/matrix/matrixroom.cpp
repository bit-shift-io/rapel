...
Error MatrixRoom::set_state(String event_type, String state_key, Dictionary event) {
  Dictionary request_body = event;

  HTTPClient::ResponseCode status = client->request_json("/_matrix/client/r0/rooms/"+room_id.http_escape()+"/state/"+event_type+"/"+state_key, request_body, HTTPClient::Method::METHOD_PUT);
...
Error MatrixRoom::get_state(String event_type, String state_key, Dictionary* event) {

    Variant response_v;
    //HTTPClient::ResponseCode status = client->request_json("/_matrix/client/r0/rooms/"+room_id.http_escape()+"/state/"+event_type+"/"+state_key, Dictionary(), HTTPClient::Method::METHOD_GET, response_v);

    // test getting all states by event
    HTTPClient::ResponseCode status = client->request_json("/_matrix/client/r0/rooms/"+room_id.http_escape()+"/state/"+event_type+"", Dictionary(), HTTPClient::Method::METHOD_GET, response_v);

    // test syncing the whole bloody room
    _state_sync(Variant());
